...
* Decoding Generated Sequence in Rust
We need to read the generated tokens from `sample_epoch_1.bin` and decode them into `[u8; 36]` token chunks using the `vocab.bin`. We can re-use logic similar to `bpe.rs` for decoding.


Ensure `rustc-hash` is available if we use `FxHashMap` - which we are already doing in the previous blocks. Let's add the basic loaders.
...
* Decoder Entry Point
This section defines the entry point for decoding a sequence binary into base tokens. By tangling to `src/decode.rs`, we keep the directory structure flat. We explicitly declare it as a binary target in `Cargo.toml`. You can run it with `cargo run --bin decode -- <sequence.bin> [vocab.bin]`.

#+begin_src rustic :tangle ./LBM_rust/src/decode.rs
mod bpe;
