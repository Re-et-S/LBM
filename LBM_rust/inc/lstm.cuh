...
struct LSTMState {
    ...
    void allocate(const LSTMConfig& cfg) {
        ...
        // gates (4x larger)
        gates_pre = std::make_unique<CudaBuffer<float>>(total_steps * 4 * cfg.hidden_dim);
        gates_post = std::make_unique<CudaBuffer<float>>(total_steps * 4 * cfg.hidden_dim);

        gates_pre_grad = std::make_unique<CudaBuffer<float>>(total_steps * 4 * cfg.hidden_dim);
...
public:
    ...
    LSTM(LSTMConfig config) : cfg(config) {
        ...
        ln_transformer.allocate(cfg.hidden_dim, cfg.batch_size, cfg.seq_length, false);
        mha.allocate(cfg);
        head.allocate(cfg);
        cublasCreate(&handle);
    }
    ...
    void predict_distribution(std::vector<float>& h_probs);
    void backward(const CudaBuffer<uint32_t>& input_tokens, const CudaBuffer<float>& grad_output);
    
    // Cross entropy
    float compute_loss_and_gradients(const CudaBuffer<uint32_t>& target_tokens, CudaBuffer<float>& grad_output);
