...
__global__ void embedding_forward_kernel(
    ...
    if (idx >= total_tokens * embedding_dim) return;
...
void LSTM::forward(const CudaBuffer<uint32_t>& input_tokens) {
    ...
    if (vec_size == 4 || vec_size == 2) {
        ...
        fallback_bias_broadcast_kernel<<<(total_preds + threads - 1) / threads, threads>>>(
            total_preds,
            D_out,
            head.b_y->get(),
            head.logits->get()
...
void LSTM::backward(const CudaBuffer<uint32_t>& input_tokens, const CudaBuffer<float>& grad_output) {
    int T = cfg.seq_length;
    int N = cfg.batch_size;
    int H = cfg.hidden_dim;
    int E = cfg.embedding_dim;
    ...
    run_transformer_causal_softmax_backward( 
        handle,
        mha,
        N,
        T 
    ...
    for (int t = T - 1; t >= 0; --t) {
        ...
        float* d_c_prev_grad = (t == 0) ? nullptr : state.c_grad->get() + (t - 1) * N * H;
        ...
        if (t > 0) {
            ...
            cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, H, N, 4 * H, &alpha,
                params.W_h->get(), 4 * H, d_gates_grad_t, 4 * H, &beta_accumulate,
                d_h_grad_prev, H);
        }
    }
    ...
    cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, 4 * H, E, T * N, 
        &alpha, state.gates_pre_grad->get(), 4 * H, embedded_input->get(), E,         
        &beta_overwrite, params.W_x_grad->get(), 4 * H);
    
    // Gradient w.r.t embedded input
    cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, E, T * N, 4 * H, 
        &alpha, params.W_x->get(), 4 * H, state.gates_pre_grad->get(), 4 * H,         
...
__global__ void softmax_distribution_kernel(
...
void LSTM::predict_distribution(std::vector<float>& h_probs) {
    int vocab_size = cfg.vocab_size;
    CudaBuffer<float> d_probs(vocab_size);
    
    // Target the last token in the first sequence of the batch
    int last_token_idx = (cfg.seq_length - 1) * cfg.batch_size + 0;
    const float* d_logits_last = head.logits->get() + last_token_idx * vocab_size;

    int threads = 256;
    softmax_distribution_kernel<<<1, threads>>>(d_logits_last, d_probs.get(), vocab_size);
    cudaDeviceSynchronize();
